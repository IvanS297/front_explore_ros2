...
Автономная исследование неизвестного пространства на основе алгоритма Frontier Exploration для ROS 2 Jazzy. Пакет реализует полный стек навигации без использования Nav2 — собственный поиск фронтиров, планировщик пути A* и контроллер движения Pure Pursuit.
...
|---------------------------------|-----------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| debug                           | false                 | Режим отладки true  → публикует /frontier_cells, /cost_map, /cspace и т.д. false → минимум топиков,                                                                                                                                            |
...
|                                           |                      |                                                                                                                                                                                       |
|-------------------------------------------|----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Параметр                                  | Значене по умолчанию | Описание                                                                                                                                                                              |
| debug                                     | false                | Режим отладки                                                                                                                                                                         |
| lookahead_distance                        | 0.35                 | Расстояние lookahead (м). Чем больше — тем плавнее путь,<br>но робот срезает углы. Чем меньше — точнее, но дёргается.<br>Рекомендуется: 0.3-0.5 для твоего робота с плохими моторами. |
| wheel_base                                | 0.19                 | Расстояние между колёсами (м). Должно совпадать с URDF.                                                                                                                               |
| max_drive_speed                           | 0.08                 | Максимальная линейная скорость (м/с).<br>Уменьши если моторы не успевают или робот нестабилен.                                                                                        |
| max_turn_speed                            | 0.8                  | Максимальная угловая скорость (рад/с).<br>Уменьши до 0.6-0.8 чтобы оба мотора успевали отработать поворот.                                                                            |
| turn_speed_kp                             | 1.25                 | Пропорциональный коэффициент угловой скорости.                                                                                                                                        |
| distance_tolerance                        | 0.1                  | Допуск до цели (м). При достижении этого расстояния до<br>конечной точки пути робот останавливается.                                                                                  |
| obstacle_avoidance_gain                   | 0.3                  | Сила отталкивания от стен. Увеличь если врезается,<br>уменьши если траектория слишком сильно искажается.                                                                              |
| obstacle_avoidance_max_slow_down_distance | 0.20                 | Расстояние (м) при котором начинается замедление.                                                                                                                                     |
| obstacle_avoidance_min_slow_down_distance | 0.12                 | Расстояние (м) при котором замедление максимальное.                                                                                                                                   |
| obstacle_avoidance_min_slow_down_factor   | 0.25                 | Минимальный коэффициент скорости при торможении у стены.<br>0.25 = замедляется до 25% от max_drive_speed.                                                                             |
| fov                                       | 200                  | Угол поля зрения (градусы). Стены в этом секторе учитываются.                                                                                                                         |
| fov_distance                              | 25                   | Радиус поля зрения (клеток карты).                                                                                                                                                    |
| fov_deadzone                              | 80                   | Мёртвая зона спереди (градусы). Стены прямо перед роботом<br>игнорируются — иначе робот не едет вперёд вдоль стены.                                                                   |
| small_fov                                 | 300                  | Малое поле зрения для близких препятствий (градусы).                                                                                                                                  |
| small_fov_distance                        | 10                   | Радиус малого поля зрения (клеток).                                                                                                                                                   |
